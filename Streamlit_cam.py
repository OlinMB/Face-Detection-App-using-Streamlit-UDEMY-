...
#Render the h1 block, contained in a frame of size 700*100
components.html("<html><body>Streamlit is an open-source Python library that makes it easy to "
...
st.markdown("<html><I>Make sure that you have Python 3.6 - Python 3.8 installed.</I><br></html>", unsafe_allow_html=True)
...
#Selection box
#first argument take the title of the selection box. Second argument takes options
How_is_streamlit = st.selectbox("liking: ",['Very much', 'So so', 'Boring', 'Useless'])
...
#Accessign haar files from a github repository
components.html('<html><body>Note: The haar cascade files can be downloaded from the '
                'a href = "https://github.com/opencv/tree/master/data/haarcascades">'
...
st.write("-> cv:CascadeClassifier::load to load a .xml classifier file. It can be either a Haar or an LBP "
         "classifier")
...
st.subheader("Step 2:")
st.write("Now the 2nd step is to load the frames and covert it into gray-scale."
         ...
         "when OpenCV reads the RGB image, it usualy stores the image in BGR (Blue, Green, Red) "
...
st.info("""faces = faceCascade.detectMutliScale(
            gray,
            scaleFactor=1.1,
            minNeighbors=5
            nimSize=(30, 30
...
st.subheader("Step 4:")
st.write("From the above step, the function detectMultiScale return 4 values - x-coordinate, "
